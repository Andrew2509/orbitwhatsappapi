...
@section('page-subtitle', 'Ubah detail paket langganan')
    ...
    <form action="{{ route('admin.plans.update', $plan) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $plan->price) }}" min="0" required 
                        class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 focus:ring-2 focus:ring-emerald-500">
            ...
            <div>
                ...
                <textarea name="description" rows="3" 
                    class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 focus:ring-2 focus:ring-emerald-500">{{ old('description', $plan->description) }}</textarea>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="number" name="max_devices" value="{{ old('max_devices', $plan->max_devices) }}" min="-1" 
                ...
                <div>
                    ...
                    <input type="number" name="max_messages_per_day" value="{{ old('max_messages_per_day', $plan->max_messages_per_day) }}" min="-1" 
                        class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700">
                    <p class="text-xs text-slate-500 mt-1">-1 untuk unlimited</p>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2">Max Kontak</label>
                    <input type="number" name="max_contacts" value="{{ old('max_contacts', $plan->max_contacts) }}" min="-1" 
                        class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700">
                    <p class="text-xs text-slate-500 mt-1">-1 untuk unlimited</p>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="feature_basic_messaging" value="1" {{ $features['basic_messaging'] ?? false ? 'checked' : '' }} class="rounded">
                    <span class="text-sm">Basic Messaging</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="feature_broadcast" value="1" {{ $features['broadcast'] ?? false ? 'checked' : '' }} class="rounded">
                    <span class="text-sm">Broadcast</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="feature_auto_reply" value="1" {{ $features['auto_reply'] ?? false ? 'checked' : '' }} class="rounded">
                    <span class="text-sm">Auto Reply</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="feature_webhook" value="1" {{ $features['webhook'] ?? false ? 'checked' : '' }} class="rounded">
                    <span class="text-sm">Webhook</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="feature_api_access" value="1" {{ $features['api_access'] ?? false ? 'checked' : '' }} class="rounded">
                    <span class="text-sm">API Access</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="feature_priority_support" value="1" {{ $features['priority_support'] ?? false ? 'checked' : '' }} class="rounded">
                    <span class="text-sm">Priority Support</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="feature_dedicated_ip" value="1" {{ $features['dedicated_ip'] ?? false ? 'checked' : '' }} class="rounded">
                    <span class="text-sm">Dedicated IP</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="feature_account_manager" value="1" {{ $features['account_manager'] ?? false ? 'checked' : '' }} class="rounded">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
            <div class="flex items-center gap-6">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="is_active" value="1" {{ $plan->is_active ? 'checked' : '' }} class="rounded">
                    <span class="text-sm font-medium">Active</span>
                </label>
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="is_featured" value="1" {{ $plan->is_featured ? 'checked' : '' }} class="rounded">
